...
def first_time_step(u, v, h, g, f, H, dt, dx, ho, gu, gh, n_grid):
    ...
    h.now[midpoint-1] =  g * H * ho * dt ** 2 / (2* dx ** 2)
    h.now[midpoint+1] =  - g * H * ho * dt ** 2 / (2* dx ** 2)


def leap_frog(u, v, h, gu, gh, f, dt, n_grid):
...
def make_graph(u, v, h, dt, n_time, n_grid):
    ...
    gridv=list(np.arange(-1,n_grid,1))
    gridv2=[i+1 for i in gridv]
    ...
    for time in range(0, n_time, interval):
        ...
        ax_u.plot(gridv2,u.store[gridv, time], color=colorVal)
        ax_v.plot(gridv2,v.store[gridv, time], color=colorVal)
...
#     n_time, n_grid = map(int, args)
    ...
    first_time_step(u, v, h, g, f, H, dt, dx, ho, gu, gh, n_grid)
    ...
    for t in np.arange(2, n_time):
        ...
        leap_frog(u, v, h, gu, gh, f, dt, n_grid)
